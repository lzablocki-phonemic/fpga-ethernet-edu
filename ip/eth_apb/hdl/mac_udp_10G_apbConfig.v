...
/*
 * UDP 10G MAC with APB Configuration
 *
 * Single Ethernet node: 10G MAC (XGMII) + UDP/IP stack + APB-configurable parameters.
 * Supports mac loopback mode.
 *
 */

module mac_udp_10G_apbConfig #
(
    parameter TARGET = "GENERIC",
    parameter BACKPRESSURE_EN = 1
)
(
    /*
     * Clock: 156.25MHz (Typical for 10G)
     * Synchronous reset
     */
    input  wire        clk,
    input  wire        rst,
    ...
    /*
     * Ethernet: XGMII (PHY-less, directly cross-connected)
     */
     ...
     */
    output wire [63:0] udp_rx_ext_tdata,
    output wire [7:0]  udp_rx_ext_tkeep,
     ...
     */
    input  wire [63:0] udp_tx_ext_tdata,
    input  wire [7:0]  udp_tx_ext_tkeep,
...
wire [63:0] rx_fifo_in_tdata;
wire [7:0]  rx_fifo_in_tkeep;
...
wire [63:0] rx_fifo_out_tdata;
wire [7:0]  rx_fifo_out_tkeep;
...
wire [63:0] tx_fifo_in_tdata;
wire [7:0]  tx_fifo_in_tkeep;
...
wire [63:0] tx_fifo_out_tdata;
wire [7:0]  tx_fifo_out_tkeep;
...
assign tx_udp_payload_axis_tkeep  = tx_fifo_out_tkeep;
...
assign rx_fifo_in_tkeep  = rx_udp_payload_axis_tkeep;
...
assign udp_rx_ext_tkeep  = rx_fifo_out_tkeep;
...
assign tx_fifo_in_tkeep  = route_loopback ? rx_fifo_out_tkeep  : udp_tx_ext_tkeep;
...
// =========================================================================
// Ethernet MAC (10G XGMII with FIFO)
// =========================================================================
eth_mac_10g_fifo #(
    .ENABLE_PADDING(1),
    .ENABLE_DIC(1),
...
eth_mac_10g_fifo_inst (
    .rx_clk(xgmii_rx_clk),
    .rx_rst(rst),
    .tx_clk(clk), // Or xgmii_tx_clk if provided as input, but usually clk for logic
    .tx_rst(rst),
    .logic_clk(clk),
    .logic_rst(rst),
...
assign xgmii_tx_clk = clk;
// synthesis translate_off

always @(posedge clk) begin
    if (rx_axis_tvalid && rx_axis_tready) begin
        $display("[Node %0s] MAC RX Packet Received", TARGET);
    end
    if (tx_axis_tvalid && tx_axis_tready && tx_axis_tlast) begin
        $display("[Node %0s] MAC TX Packet Sent", TARGET);
    end
end
// synthesis translate_on
...
localparam ARP_RETRY_INTERVAL = 156250000 * 1;  
localparam ARP_TIMEOUT        = 156250000 * 4;  
...
// =========================================================================
// RX FIFO — Buffers incoming UDP payload
// =========================================================================
axis_fifo #(
    .DEPTH(8192),
    .DATA_WIDTH(64),
...
udp_rx_fifo (
    ...
    .m_axis_tkeep (rx_fifo_out_tkeep),
...
// =========================================================================
// TX FIFO — Buffers outgoing UDP payload
// =========================================================================
axis_fifo #(
    .DEPTH(8192),
    .DATA_WIDTH(64),
...
udp_tx_fifo (
    ...
    .m_axis_tkeep (tx_fifo_out_tkeep),
